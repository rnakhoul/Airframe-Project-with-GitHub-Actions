function diffGitHub_pullrequest(branchname)
    ...
    gitCommand = sprintf('git diff --name-only $(git merge-base --fork-point main %s)..%s ***.slx',branchname, branchname);
...
function ancestor = getAncestor(tempdir,fileName,branchname)
    ...
    gitCommand = sprintf('git show $(git merge-base --fork-point main %s):%s > %s', branchname, fileName, ancestor);
